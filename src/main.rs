//! riddle — the diary of Tom Riddle, ported for the reMarkable 1.
//!
...
usage:
  riddle                      open the diary (windowed when AppLoad sets
                              QTFB_KEY, otherwise native RM1 takeover)
...
fn run() -> std::io::Result<()> {
    ...
    let takeover = matches!(disp, display::Display::Rm1(_));
    eprintln!(
        "riddle: display {} ({}x{} stride {})",
        if takeover { "rm1/native" } else { "qtfb" },
        surf.w,
        surf.h,
        surf.stride
    );
